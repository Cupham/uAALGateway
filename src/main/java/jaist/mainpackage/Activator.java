...
import jaist.ContextBus.CPublisher;
import jaist.ServiceBus.SCallee_SmartEnvironment;
import jaist.echonet.objects.EchonetLiteDevice;
import jaist.echonet.objects.HomeResourceObserver;
import jaist.echonet.objects.NodeProfileObject;
import jaist.echonet.objects.eAirConditioner;
import jaist.echonet.objects.eDataObject;
import jaist.echonet.objects.eTemperatureSensor;
import jaist.homegateway.services.EchonetDeviceScanner;
import jaist.homegateway.services.EchonetDeviceUpdater;
import jaist.utils.OntologyHelper;
import jaist.utils.SetUtilities;
...
public class Activator implements BundleActivator {
	...
	private EchonetOntology echonet_ontology = new EchonetOntology();
	public static long currentTime = 0;
	public static BundleContext osgiContext = null;
	public static ModuleContext context = null;	
	public static int numberOfDevice = 0;
	public static int numberOfNode = 0;
    int numberofSensor = 0;
    int numberofAircon = 0;
    int numberofRun=0;
	...
	public void start(BundleContext bcontext) throws Exception {
		...
		timer.schedule(new TimerTask() {
			...
			public void run() {
				numberofRun +=1;
				LOGGER.info("Run: " + numberofRun + " Published: " + changedOntologies.size());
				System.out.println("------------------RUN: " + numberofRun + " ------------------");
		...
		if(changedOntologies.size() == 0) {
			...
			for(String URI :changedOntologies) {
				if(URI.contains("I_Temperature")) {
					...
					ContextEvent lighting  = new ContextEvent(Activator.i_Lighting, GeneralLighting.MY_URI);
					cPublisher.publicContextEvent(lighting);
					SetUtilities.removeIfExistFromSet(Activator.changedOntologies, URI);
				}else if(URI.contains("I_Consent")) {
					Activator.i_Consent = consentOntologies.get(URI);
					ContextEvent consent  = new ContextEvent(Activator.i_Consent, Switch.MY_URI);
			...
			monitor.addMonitorListener(new MonitorListener() {
	            ...
	            public void detectEOJsJoined(Monitor monitor, Node node, List<EOJ> eojs) {
	                ...
	                for(EOJ eoj :  eojs) {
	                	...
	                	numberOfDevice +=1;
	                	long time_detect  = System.currentTimeMillis() - start_time;
	                	if((eoj.getClassCode() == (byte) 0x11) && (eoj.getClassGroupCode() == 0x00))
	                		numberofSensor+=1;
	                	if((eoj.getClassCode() == (byte) 0x30) && (eoj.getClassGroupCode() == 0x01))
	                		numberofAircon+=1;
	                	       
                		total_time  +=time_detect;
	                	LOGGER.info("Total Time " + total_time +" Time detect " + time_detect +" ms "+" AIRCOn " + numberofAircon +" TEMP " + numberofSensor + "  " +numberOfNode + " _ " + numberOfDevice + ": "+ node + ":" +eoj);
	                	if(eoj.isProfileObject()) {
	                		...
	                		try {
	                			...
	                			System.out.println("Number of Device = " +numberOfDevice);
