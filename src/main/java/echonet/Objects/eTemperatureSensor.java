...
public class eTemperatureSensor extends eDataObject{
	...
	public void refreshTemperature(float temperature) {
		if(this.getTemperature() != temperature) {
			...
			sensor.setMeasuredTemperatureValue(new Float(temperature));
	...
	public void refreshOperationStatus(boolean operationStatus) {
		if(this.getOperationStatus() != operationStatus) {
			...
			if(operationStatus) {
				sensor.setOperationStatus(OperationStatusValue.On);
			} else {
				sensor.setOperationStatus(OperationStatusValue.Off);
			}
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_OPERATION_STATUS));
		}
	...
	public void refreshInstallLocation(String installLocation) {
		if(!installLocation.equals(this.getInstallLocation())) {
			...
			sensor.setInstallationLocation(EchonetDataConverter.installationLocationToEnum(installLocation));
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_INSTALLATION_LOCATION));
		}
	}
	public void refreshStandardVersionInfo(String standardVersionInfo) {
		if(!standardVersionInfo.equals(this.getStandardVersionInfo())) {
			...
			sensor.setStandardVersionInformation(standardVersionInfo);
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_STANDARD_VERSION_INFORMATION));
		}		
	}
	public void refreshIdentificationNumber(String identificationNumber) {
		if(!identificationNumber.equals(this.getIdentificationNumber())) {
			...
			sensor.setIdentificationNumber(EchonetDataConverter.stringToIdentificationNumber(identificationNumber));
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_IDENTIFICATION_NUMBER));
		}
	...
	public void refreshInstantaneousPower(short instantaneousPower) {
		if(this.getInstantaneousPower() != instantaneousPower) {
			...
			sensor.setMeasuredInstantaneousPowerConsumption(new Integer(instantaneousPower));
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_MEASURED_INSTANTANEOUS_POWER_CONSUMPTION));
		}
	...
	public void refreshCumulativePower(long cumulativePower) {
		if(this.getCumulativePower() != cumulativePower) {
			...
			sensor.setMeasuredCumulativePowerConsumption(new Float(cumulativePower));
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_MEASURED_CUMULATIVE_POWER_CONSUMPTION));
		}	
	}
	public void refreshManufactureerFaultCode(String manufactureerFaultCode) {
		if(!manufactureerFaultCode.equals(this.getManufacturerFaultCode())) {
			...
			sensor.setManufacturerFaultCode(manufactureerFaultCode);
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_MANUFACTURER_FAULT_CODE));
		}	
	}
	public void refreshCurrentLimitSetting(int currentLimitSetting) {
		if(this.getCurrentLimitSetting()!=currentLimitSetting) {
			...
			sensor.setCurrentLimitSetting(new Integer(currentLimitSetting));
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_CURRENT_LIMIT_SETTING));
		}	
	}
	public void refreshFaultStatus(boolean faultStatus) {
		if(this.isFaultStatus() != faultStatus) {
			...
			if(faultStatus) {
				sensor.setFaultStatus(OccurenceStatusValue.OccurenceStatusFound);
			} else {
				sensor.setFaultStatus(OccurenceStatusValue.OccurenceStatusNotFound);
			}	
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_FAULT_STATUS));
		}	
	}
	public void refreshFaultDescription(String faultDescription) {
		if(!faultDescription.equals(this.getFaultDescription())) {
			this.faultDescription = faultDescription;
			sensor.setFaultDesciptionValue(EchonetDataConverter.stringToFaultDescription(faultDescription));
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_FAULT_DESCRIPTION));
		}
	}
	public void refreshManufacturerCode(String manufacturerCode) {
		if(!manufacturerCode.equals(this.getManufacturerCode())) {
			this.manufacturerCode = manufacturerCode;
			sensor.setManufacturerCode(manufacturerCode);
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_MANUFACTURER_CODE));
		}		
	}
	public void refreshBusinessFacilityCode(String businessFacilityCode) {
		if(!businessFacilityCode.equals(this.getBusinessFacilityCode())) {
			...
			sensor.setBusinessFacilityCode(businessFacilityCode);
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_BUSINESS_FACILITY_CODE));
		}	
	}
	public void refreshProductCode(String productCode) {
		if(!productCode.equals(this.getProductCode())) {
			this.productCode = productCode;
			sensor.setProductCode(productCode);
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_PRODUCT_CODE));
		}	
	}
	public void refreshProductNumber(String productNumber) {
		if(!productNumber.equals(this.getProductNumber())) {
			this.productNumber = productNumber;
			sensor.setProductionNumber(productNumber);
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_PRODUCTION_NUMBER));
		}	
	}
	public void refreshProductDate(Date productDate) {
		if(!productDate.equals(this.getProductDate())) {
			this.productDate = productDate;
			sensor.setProductionString(productDate.toString());
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_PRODUCTION_DATE));
		}
	...
	public void refreshPowerSaving(boolean powerSaving) {
		if(this.getPowerSaving()!=powerSaving) {
			...
			if(powerSaving) {
				sensor.setPowerSavingOperationSetting(OperationModeSettingValue.PowerSavingMode);
			} else {
				sensor.setPowerSavingOperationSetting(OperationModeSettingValue.NormalMode);
			}
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_POWER_SAVING_OPERATION_SETTING));
		}
	...
	public void refreshThroughPublicNetwork(boolean throughPublicNetwork) {
		if(this.getThroughPublicNetwork()!=throughPublicNetwork) {
			...
			if(throughPublicNetwork) {
				sensor.setRemoteControlSetting(RemoteControlSettingValue.ThroughPublicNetwork);
			} else {
				sensor.setRemoteControlSetting(RemoteControlSettingValue.NotThroughPublicNetwork);
			}
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_REMOTE_CONTROL_SETTING));
		}
	...
	public void refreshCurrentTimeSetting(String currentTimeSetting) {
		if(!currentTimeSetting.equals(this.getCurrentTimeSetting())) {
			...
			sensor.setCurrentTimeSetting(currentTimeSetting);
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_CURRENT_TIME_SETTING));
		}
	}
	public void refreshCurrentDateSetting(Date currentDateSetting) {
		if(!currentDateSetting.equals(this.getCurrentDateSetting())) {
			...
			sensor.setCurrentStringSetting(currentDateSetting.toString());
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_CURRENT_DATE_SETTING));
		}	
	}
	public void refreshPowerLimit(int powerLimit) {
		if(this.getPowerLimit()!=powerLimit) {
			this.powerLimit = powerLimit;
			sensor.setPowerLimitSetting(new Integer(powerLimit));
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_POWER_LIMIT_SETTING));
		}
	}
	public void refreshCumulativeTime(String cumulativeTime) {
		if(!cumulativeTime.equals(this.getCumulativeTime())) {
			this.cumulativeTime = cumulativeTime;
			sensor.setCumulativeOperatingTime(cumulativeTime);
			Activator.cPublisher.publicContextEvent(new ContextEvent(sensor,TemperatureSensor.PROPERTY_HAS_CUMULATIVE_OPERATING_TIME));
		}
	}
			...
			service.doGet(node, eoj, epcs, 5000, new GetListener() {
			    ...
			    public void receive(GetResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					case xE0:
						...
						LOGGER.info(String.format("Node:%s@EOJ:%s {EPC:0xE0, EDT: 0x%02X}=={Temperature:%.2f}",
