...
public class eTV extends eDataObject{
	private static final Logger LOGGER = Logger.getLogger(eTV.class.getName());
	private Timer timer;
	...
	public eTV(byte instanceCode) {
		super();
		this.groupCode= (byte) 0x00;
		this.classCode = (byte) 0x22;
		this.instanceCode = instanceCode;
	}
	public eTV(EOJ eoj, Node node) {
		super(node, eoj);
		this.groupCode= (byte) 0x00;
		this.classCode = (byte) 0x22;
		this.instanceCode = eoj.getInstanceCode();
		tv = new Television(Television.MY_URI+getDeviceID());
	...
	// Provided Services
				public boolean setDeviceLocation(String location) {
					...
					if(location.equals(getInstallLocation())) {
						LOGGER.info(String.format("TV is already in %s nothing to do", location));
						rs = true;
					} else {
				...
				public boolean setOn() {
					...
					if(getOperationStatus()) {
						LOGGER.info("TV is already ON! nothing to do");
						rs = true;
					} else {
				...
				public boolean setOff() {
					boolean rs = false;
					if(!getOperationStatus()) {
						LOGGER.info("TV is already OFF! nothing to do");
						rs = true;
					} else {
			...
			public void refreshOperationStatus(boolean operationStatus) {
				if(this.getOperationStatus() != operationStatus) {
					...
					if(operationStatus) {
						tv.setOperationStatus(OperationStatusValue.On);
					} else {
						tv.setOperationStatus(OperationStatusValue.Off);
					}
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_OPERATION_STATUS));
				}
			...
			public void refreshInstallLocation(String installLocation) {
				if(!installLocation.equals(this.getInstallLocation())) {
					...
					tv.setInstallationLocation(EchonetDataConverter.installationLocationToEnum(installLocation));
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_INSTALLATION_LOCATION));
				}
			}
			public void refreshStandardVersionInfo(String standardVersionInfo) {
				if(!standardVersionInfo.equals(this.getStandardVersionInfo())) {
					...
					tv.setStandardVersionInformation(standardVersionInfo);
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_STANDARD_VERSION_INFORMATION));
				}		
			}
			public void refreshIdentificationNumber(String identificationNumber) {
				if(!identificationNumber.equals(this.getIdentificationNumber())) {
					...
					tv.setIdentificationNumber(EchonetDataConverter.stringToIdentificationNumber(identificationNumber));
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_IDENTIFICATION_NUMBER));
				}
			...
			public void refreshInstantaneousPower(short instantaneousPower) {
				if(this.getInstantaneousPower() != instantaneousPower) {
					...
					tv.setMeasuredInstantaneousPowerConsumption(new Integer(instantaneousPower));
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_MEASURED_INSTANTANEOUS_POWER_CONSUMPTION));
				}
			...
			public void refreshCumulativePower(long cumulativePower) {
				if(this.getCumulativePower() != cumulativePower) {
					...
					tv.setMeasuredCumulativePowerConsumption(new Float(cumulativePower));
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_MEASURED_CUMULATIVE_POWER_CONSUMPTION));
				}	
			}
			public void refreshManufactureerFaultCode(String manufactureerFaultCode) {
				if(!manufactureerFaultCode.equals(this.getManufacturerFaultCode())) {
					...
					tv.setManufacturerFaultCode(manufactureerFaultCode);
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_MANUFACTURER_FAULT_CODE));
				}	
			}
			public void refreshCurrentLimitSetting(int currentLimitSetting) {
				if(this.getCurrentLimitSetting()!=currentLimitSetting) {
					...
					tv.setCurrentLimitSetting(new Integer(currentLimitSetting));
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_CURRENT_LIMIT_SETTING));
				}	
			}
			public void refreshFaultStatus(boolean faultStatus) {
				if(this.isFaultStatus() != faultStatus) {
					...
					if(faultStatus) {
						tv.setFaultStatus(OccurenceStatusValue.OccurenceStatusFound);
					} else {
						tv.setFaultStatus(OccurenceStatusValue.OccurenceStatusNotFound);
					}	
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_FAULT_STATUS));
				}	
			}
			public void refreshFaultDescription(String faultDescription) {
				if(!faultDescription.equals(this.getFaultDescription())) {
					this.faultDescription = faultDescription;
					tv.setFaultDesciptionValue(EchonetDataConverter.stringToFaultDescription(faultDescription));
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_FAULT_DESCRIPTION));
				}
			}
			public void refreshManufacturerCode(String manufacturerCode) {
				if(!manufacturerCode.equals(this.getManufacturerCode())) {
					this.manufacturerCode = manufacturerCode;
					tv.setManufacturerCode(manufacturerCode);
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_MANUFACTURER_CODE));
				}		
			}
			public void refreshBusinessFacilityCode(String businessFacilityCode) {
				if(!businessFacilityCode.equals(this.getBusinessFacilityCode())) {
					...
					tv.setBusinessFacilityCode(businessFacilityCode);
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_BUSINESS_FACILITY_CODE));
				}	
			}
			public void refreshProductCode(String productCode) {
				if(!productCode.equals(this.getProductCode())) {
					...
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_PRODUCT_CODE));
				}	
			}
			public void refreshProductNumber(String productNumber) {
				if(!productNumber.equals(this.getProductNumber())) {
					this.productNumber = productNumber;
					tv.setProductionNumber(productNumber);
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_PRODUCTION_NUMBER));
				}	
			}
			public void refreshProductDate(Date productDate) {
				if(!productDate.equals(this.getProductDate())) {
					this.productDate = productDate;
					tv.setProductionString(productDate.toString());
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_PRODUCTION_DATE));
				}
			...
			public void refreshPowerSaving(boolean powerSaving) {
				if(this.getPowerSaving()!=powerSaving) {
					...
					if(powerSaving) {
						tv.setPowerSavingOperationSetting(OperationModeSettingValue.PowerSavingMode);
					} else {
						tv.setPowerSavingOperationSetting(OperationModeSettingValue.NormalMode);
					}
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_POWER_SAVING_OPERATION_SETTING));
				}
			...
			public void refreshThroughPublicNetwork(boolean throughPublicNetwork) {
				if(this.getThroughPublicNetwork()!=throughPublicNetwork) {
					...
					if(throughPublicNetwork) {
						tv.setRemoteControlSetting(RemoteControlSettingValue.ThroughPublicNetwork);
					} else {
						tv.setRemoteControlSetting(RemoteControlSettingValue.NotThroughPublicNetwork);
					}
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_REMOTE_CONTROL_SETTING));
				}
			...
			public void refreshCurrentTimeSetting(String currentTimeSetting) {
				if(!currentTimeSetting.equals(this.getCurrentTimeSetting())) {
					...
					tv.setCurrentTimeSetting(currentTimeSetting);
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_CURRENT_TIME_SETTING));
				}
			}
			public void refreshCurrentDateSetting(Date currentDateSetting) {
				if(!currentDateSetting.equals(this.getCurrentDateSetting())) {
					...
					tv.setCurrentStringSetting(currentDateSetting.toString());
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_CURRENT_DATE_SETTING));
				}	
			}
			public void refreshPowerLimit(int powerLimit) {
				if(this.getPowerLimit()!=powerLimit) {
					this.powerLimit = powerLimit;
					tv.setPowerLimitSetting(new Integer(powerLimit));
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_POWER_LIMIT_SETTING));
				}
			}
			public void refreshCumulativeTime(String cumulativeTime) {
				if(!cumulativeTime.equals(this.getCumulativeTime())) {
					this.cumulativeTime = cumulativeTime;
					tv.setCumulativeOperatingTime(cumulativeTime);
					Activator.cPublisher.publicContextEvent(new ContextEvent(tv,Television.PROPERTY_HAS_CUMULATIVE_OPERATING_TIME));
				}
			}
